...
--no core --------------------------------------------------------
 ...
 IF @v_re_evaluar = 1  
 BEGIN  
  INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'I','   Inicia reasimilacion regla: '+ STR(@c_reg_id), @v_usu_id, Null,@cat_id  
  EXEC dbo.COM_wf_p_reasimilacion_escenarios   
   ...
   @cat_id,  
  @v_cod_ret  
  INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'F','   Finaliza reasimilacion regla: '+ STR(@c_reg_id), @v_usu_id, Null,@cat_id  
 END  
