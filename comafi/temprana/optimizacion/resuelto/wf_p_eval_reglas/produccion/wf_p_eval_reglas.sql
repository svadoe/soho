ALTER PROCEDURE [dbo].[wf_p_eval_reglas] (   
...
AS  
/*  
20200420 sergio vado espinoza: Se agregan mensajes a ser registrados en la bitácora de la tabla
                               wf_print_out: inicio de ejecución de regla e inicio y fin de 
                               ejecución de regla del proceso de reasignación de escenarios.
...
BEGIN  
 ...
 BEGIN  
  ...
  END 
  --20181018 - Modificacion para ejecucion de asignaciones luego de reevaluación
  IF EXISTS (SELECT TOP 1 1 FROM wf_eventos_x_reg WHERE exr_reg=@c_reg_id AND exr_baja_fecha IS NULL AND exr_evento IN (520,521,522)) 
	SELECT @v_cons = @v_cons + ' NOT EXISTS (SELECT TOP 1 1 FROM wf_eventos_proceso WHERE evp_sob=cta_id AND evp_evento IN (520,521,522)) AND (cta_lockeo_fec_hora IS NULL OR cta_lockeo_fec_hora =  ''' +   LTRIM(RTRIM( CONVERT(char, @v_fec_proceso,112)))   + ''') AND esc_susp = ''N'' AND est_inactivo = ''N'' AND '  + @c_reg_query  
  ELSE
 ...
 BEGIN  
  ...
  --20181018 - Modificacion para ejecucion de asignaciones luego de reevaluación
  IF EXISTS (SELECT TOP 1 1 FROM wf_eventos_x_reg WHERE exr_reg=@c_reg_id AND exr_baja_fecha IS NULL AND exr_evento IN (520,521,522))  
	SELECT @v_cons = @v_cons + ' AND NOT EXISTS (SELECT TOP 1 1 FROM wf_eventos_proceso WHERE evp_sob=cta_id AND evp_evento IN (520,521,522)) AND (cta_lockeo_fec_hora IS NULL OR cta_lockeo_fec_hora =  ''' +   LTRIM(RTRIM( CONVERT(char, @v_fec_proceso,112)))   + ''') AND esc_susp = ''N'' AND est_inactivo = ''N'' AND ' + @c_reg_query  
  ELSE
...
--select @sql as cursor1  
--select @v_cons as select1
--exec  (@v_cons)
...
--no core --------------------------------------------------------
 ...
 OPEN #cur_cta  
--select @@CURSOR_ROWS as 'cuentas atrapadas' 
--no core --------------------------------------------------------
...
--no core --------------------------------------------------------
 ...
 BEGIN  
  /*20200420 sergio vado espinoza*/
  INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'I','Inicia regla de reasimilacion de escenarios: '+ STR(@c_reg_id), @v_usu_id, Null,@cat_id  

  EXEC dbo.wf_p_reasimilacion_escenarios   
   @v_fec_proceso,   
   @v_usu_id,  
   @c_reg_id,  
   @cat_id,  
  ...
  /*20200420 sergio vado espinoza*/
  INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'F','Finaliza regla de reasimilación de escenarios: '+ STR(@c_reg_id), @v_usu_id, Null,@cat_id  
 END  
