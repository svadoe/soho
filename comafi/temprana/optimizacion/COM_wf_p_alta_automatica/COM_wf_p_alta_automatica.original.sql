...
BEGIN
	...
	DECLARE @tpa_id int
	DECLARE @cat_id int
	Declare rsBitacora Cursor For	Select	alt_id,
						...
						cta_civ = ISNULL((select civ_id from cond_iva where civ_nombre_corto = incta_civ and civ_baja_fecha is null),0), 
						cta_age = ISNULL((select age_id from agencias where age_cod = incta_age and age_baja_fecha is null),0) ,
					...
					WHERE
						...
						alt_tabla='cta'
	Open rsBitacora
	Fetch Next From rsBitacora into @cta_id,@pro_id,@cla_id,@suc_id,@mon_id,@cnl_id,@cam_id,@bnc_id,@ent_id,@civ_id,@age_id,@tco_id,@tpa_id,@cat_id
	While @@Fetch_Status = 0
	BEGIN
		--Print @cta_id
		/* Se elimina el cambio de cartera ya que no existen casos con este comportamiento*/
		/*EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_cat',  --@v_tipo		varchar(20),
			@cat_id, --@v_id_nuevo	int,
		...
		*/
		...
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_pro',  --@v_tipo		varchar(20),
			@pro_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_cla',  --@v_tipo		varchar(20),
			@cla_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_suc',  --@v_tipo		varchar(20),
			@suc_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_bnc',  --@v_tipo		varchar(20),
			@bnc_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
	/* INI - se eliminan estos cambios ya que no son siginificativo el cambio y demora el batch - 20200408
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_mon',  --@v_tipo		varchar(20),
			@mon_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_cnl',  --@v_tipo		varchar(20),
			@cnl_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_cam',  --@v_tipo		varchar(20),
			@cam_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_ent',  --@v_tipo		varchar(20),
			@ent_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_civ',  --@v_tipo		varchar(20),
			@civ_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
	/*	EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_age',  --@v_tipo		varchar(20),
			@age_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT     SE ACTUALIZAN LAS AGENCIAS POR REGLAS   */
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_tco',  --@v_tipo		varchar(20),
			@tco_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		EXEC dbo.cmb_cta_auto
			@cta_id,  --@v_cta_id	int,
			'cta_tpa',  --@v_tipo		varchar(20),
			@tpa_id, --@v_id_nuevo	int,
			@v_usu_id,          --@v_usu_id	  	int,
			'',
			@error_aux  --@v_cod_ret	int OUT
		*/ -- FIN - se eliminan estos cambios ya que no son siginificativo el cambio y demora el batch - 20200408
		Fetch Next From rsBitacora into @cta_id,@pro_id,@cla_id,@suc_id,@mon_id,@cnl_id,@cam_id,@bnc_id,@ent_id,@civ_id,@age_id,@tco_id,@tpa_id,@cat_id
	END
	Close rsBitacora
	Deallocate rsBitacora
